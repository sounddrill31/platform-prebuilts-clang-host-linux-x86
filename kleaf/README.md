...
User toolchains, named
`user_{target_os}_{target_cpu}_clang_toolchain`,
...
These toolchains have `cc_toolchain.compiler` set to `"unknown"`.
...
1.  If `--user_clang_toolchain` is set, the user toolchains are registered
    via `--extra_toolchains` ([source](user_toolchains.bazelrc))
...
1.  If `--user_clang_toolchain` is set, `user_linux_x86_64_clang_toolchain`
    is returned because its contraint values (`linux`, `x86_64`) are a subset
...
3.  The default toolchain `linux_x86_64_clang_toolchain` is returned because
    its contraint values (`linux`, `x86_64`) are a subset
...
1.  If `--user_clang_toolchain` is set, `user_android_arm64_clang_toolchain`
    is returned because its contraint values (`android`, `arm64`) are a subset
...
3.  The default toolchain `android_arm64_clang_toolchain` is returned because
    its contraint values (`android`, `arm64`) are a subset
...
*   If `--user_clang_toolchain` is set, the user toolchain
    `user_linux_x86_64_clang_toolchain` is returned because its contraint
...
*   If `--user_clang_toolchain` is not set, the matching versioned toolchain,
    ...
    its contraint values (`linux`, `x86_64` `{toolchain_version}`) are a subset
...
*   If `--user_clang_toolchain` is set, the user toolchain
    ...
    contraint values (`android`, `{target_cpu}`) are a subset of the target
    platform's constraint values
    (`android`, `{target_cpu}`, `{toolchain_version}`). However,
    `kernel_toolchains` rejects the user toolchain because the version
    `"unknown"` does not match the declared value,
    `kernel_build.toolchain_version`, resulting in a build error. You should
    delete `kernel_build.toolchain_version`, and try again.
*   If `--user_clang_toolchain` is not set, the matching versioned toolchain,
    ...
    its contraint values (`android`, `{target_cpu}` `{toolchain_version}`) are a subset of the target platform's constraint values
...
*   An environment variable, `KLEAF_USER_CLANG_TOOLCHAIN_PATH`, must
    be set to the path to the user clang toolchain.
*   The workspace file must call `register_clang_toolchains()` with
    arguments to `user_clang_toolchain_repository`. Example:
    ```
    register_clang_toolchains(
        user_clang_toolchain_local_symlink = "@kleaf_user_clang_toolchain//clang_user",
    )
    ```
*   The following flag must be specified to ignore any existing `BUILD` file
    under the user clang toolchain path.
    ```
    # Replace according to the arguments to register_clang_toolchains()
    --deleted_packages=@kleaf_user_clang_toolchain//clang_user
    ```
