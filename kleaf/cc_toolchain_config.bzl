...
def _tool_paths(ctx):
    ...
    # Using symlink to find the binary because tool_path only accept relative
    # paths to cc_toolchain's package; see
...
def _features(ctx, sysroot):
    return [
        _toolchain_include_feature(
            name = "toolchain_system_include_directories",
            include_flag = "-isystem",
            includes = _system_include_dirs(ctx)
        ),
        _toolchain_include_feature(
            name = "toolchain_include_directories",
            include_flag = "-I",
            includes = _include_dirs(sysroot = sysroot)
        ),
        _host_ldflags(ctx),
        _common_ldflags(ctx),
    ]
...
def _impl(ctx):
    return cc_common.create_cc_toolchain_config_info(
        ...
        features = _features(ctx, _sysroot()),
        builtin_sysroot = _sysroot(),
        # This says "cxx" but apparently applies to C as well
#        cxx_builtin_include_directories = _include_dirs(_sysroot()),
