...
def _user_clang_toolchain_repository_impl(repository_ctx):
    if "KLEAF_USER_CLANG_TOOLCHAIN_PATH" not in repository_ctx.os.environ:
        ...
        return
    repository_ctx.symlink(repository_ctx.os.environ["KLEAF_USER_CLANG_TOOLCHAIN_PATH"], "clang-user")
...
_user_clang_toolchain_repository = repository_rule(
...
def user_clang_toolchain_repository():
    """Defines a repository that provides a clang version at a user defined path."""
    _user_clang_toolchain_repository(
