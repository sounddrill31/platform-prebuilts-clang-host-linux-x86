...
def _android_ldflags(ndk_triple):
    # From _setup_env.sh
    # USERLDFLAGS
    return feature(
...
def _android_cflags(ndk_triple):
    # From _setup_env.sh
    # USERCFLAGS
    return feature(
        name = "kleaf-android-ldflags",
...
def _android_features(ctx):
    if ctx.attr.ndk_triple:
        return [
            _android_ldflags(ctx.attr.ndk_triple),
            _android_cflags(ctx.attr.ndk_triple),
        ]
    return []

android = struct(
    features = _android_features,
)
