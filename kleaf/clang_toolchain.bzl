...
def clang_toolchain(
    ...
    # Whether we want to wrap labels relative to common_prefix
    # with Label() cnstructor
    wrap = _identity
    if not common_prefix:
        common_prefix = "//prebuilts/clang/host/linux-x86/clang-{}:".format(clang_version)
        wrap = _wrap_label

    if not includes:
        includes = wrap("{}includes".format(common_prefix))
    ...
    if not all_binaries:
        all_binaries = wrap("{}binaries".format(common_prefix))
    ...
    clang = wrap("{}bin/clang".format(common_prefix))
    clang_plus_plus = wrap("{}bin/clang++".format(common_prefix))
    ld = clang
    strip = wrap("{}bin/llvm-strip".format(common_prefix))
    ar = wrap("{}bin/llvm-ar".format(common_prefix))
    objcopy = wrap("{}bin/llvm-objcopy".format(common_prefix))
