...
def _thinlto_test_impl(ctx):
    ...
    for action in actions:
        if action.mnemonic in ctx.attr.expected_actions:
...
thinlto_test = analysistest.make(
    ...
    attrs = {
        ...
        "expected_actions": attr.string_list(
...
def test_thin_lto_feature():
    ...
    thinlto_test(
        ...
        expected_actions = ["CppCompile", "CppLink"],
        expected_flags = [
            "-flto=thin",
            "-fsplit-lto-unit",
        ],
...
def test_whole_program_vtables_feature():
    ...
    thinlto_test(
        name = test_name,
        target_under_test = name,
        expected_actions = ["CppLink"],
        expected_flags = ["-fwhole-program-vtables"],
    )

    return test_name

def test_whole_program_vtables_requires_thinlto():
...
def test_limit_cross_tu_inline_requires_thinlto():
...
def cc_toolchain_features_lto_test_suite(name):
    native.test_suite(
        ...
        tests = [
            ...
            test_whole_program_vtables_requires_thinlto(),
            test_limit_cross_tu_inline_feature(),
            test_limit_cross_tu_inline_requires_thinlto(),
