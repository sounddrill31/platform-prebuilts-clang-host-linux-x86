...
func llvmPrebuiltLibraryStatic(ctx android.LoadHookContext) {
	...
	if name == "libsimpleperf_readelf.a" {
		...
		p.Target.Darwin_x86_64.Srcs = []string{":libsimpleperf_readelf_darwin"}
		p.Target.Darwin_arm64.Srcs = []string{":libsimpleperf_readelf_darwin"}
	} else {
