...
func setAndroidLibcxxSrcProps(ctx android.LoadHookContext, ap *archInnerProps, arch string,
	libName string) {

	clangDir := getClangPrebuiltDir(ctx)
	setSrcsField := func(field *[]string, kind string) {
		*field = []string{path.Join(clangDir, "android_libc++", kind, arch, "lib", libName)}
	}

	// TODO: Remove this code once the toolchain prebuilt has http://aosp/2924821 and http://aosp/2924829.
	if android.ExistentPathForSource(ctx, "prebuilts/clang/host/linux-x86", clangDir,
		"android_libc++", "apex", arch, "lib", libName).Valid() {
		...
		setSrcsField(&ap.Srcs, "apex")
		return
	}

	if arch == "aarch64" {
		setSrcsField(&ap.Sanitized.None.Srcs, "platform")
		setSrcsField(&ap.Sanitized.Hwaddress.Srcs, "platform_hwasan")
	} else {
		setSrcsField(&ap.Srcs, "platform")
	}
}
...
func llvmPrebuiltLibraryShared(ctx android.LoadHookContext) {
	...
	if moduleName == "libc++" {
		setAndroidProps := func(ap *archInnerProps, arch string) {
			setAndroidLibcxxSrcProps(ctx, ap, arch, "libc++.so")
			ap.Export_include_dirs = androidLibcxxHeaderDirs(ctx, arch)
		}
		setAndroidProps(&p.Target.Android_arm, "arm")
		setAndroidProps(&p.Target.Android_arm64, "aarch64")
		setAndroidProps(&p.Target.Android_riscv64, "riscv64")
		setAndroidProps(&p.Target.Android_x86, "i386")
		setAndroidProps(&p.Target.Android_x86_64, "x86_64")
		setAndroidProps(&p.Target.Linux_bionic_arm64, "aarch64")
		setAndroidProps(&p.Target.Linux_bionic_x86_64, "x86_64")
...
func llvmPrebuiltLibraryStatic(ctx android.LoadHookContext) {
	...
	if moduleName == "libc++_static" {
		setAndroidProps := func(ap *archInnerProps, arch string) {
			setAndroidLibcxxSrcProps(ctx, ap, arch, "libc++_static.a")
			ap.Export_include_dirs = androidLibcxxHeaderDirs(ctx, arch)
		}
		setAndroidProps(&p.Target.Android_arm, "arm")
		setAndroidProps(&p.Target.Android_arm64, "aarch64")
		...
		setAndroidProps := func(ap *archInnerProps, arch string) {
			setAndroidLibcxxSrcProps(ctx, ap, arch, "libc++demangle.a")
		}
		setAndroidProps(&p.Target.Android_arm, "arm")
		setAndroidProps(&p.Target.Android_arm64, "aarch64")
		setAndroidProps(&p.Target.Android_riscv64, "riscv64")
