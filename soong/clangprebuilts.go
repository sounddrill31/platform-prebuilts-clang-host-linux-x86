...
func libClangRtPrebuiltLibraryShared(ctx android.LoadHookContext, libProps *prebuiltLibraryProps,
	...
	p.Target.Glibc_x86.Srcs = []string{path.Join(libDir, name+"-i686"+suffix+".so")}
	p.Target.Glibc_x86.Stem = proptools.StringPtr(name + "-i686" + suffix)
...
func libClangRtPrebuiltLibraryStatic(ctx android.LoadHookContext, libProps *prebuiltLibraryProps) {
	...
	p.Target.Glibc_x86.Srcs = []string{path.Join(libDir, name+"-i686"+suffix+".a")}
