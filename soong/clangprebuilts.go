...
func androidLibcxxHeaderDirs(ctx android.LoadHookContext, arch string) []string {
	...
	return []string{
		path.Join(clangDir, "android_libc++", "apex", arch, "include", "c++", "v1"),
...
func llvmPrebuiltLibraryShared(ctx android.LoadHookContext) {
	...
	if moduleName == "prebuilt_libc++" {
		makeAndroidSrcs := func(arch string) []string {
			// Always use the APEX variant, which is built for the older API level,
			// because this prebuilt is used for both APEX and system binaries.
			return []string{path.Join(clangDir, "android_libc++", "apex",
				arch, "lib", "libc++.so")}
		}
		makeHostSrcs := func(triple string) []string {
			return []string{path.Join(clangDir, "lib", triple, libcxxSoName)}
		}
		p.Target.Android_arm.Srcs = makeAndroidSrcs("arm")
		p.Target.Android_arm.Export_include_dirs = androidLibcxxHeaderDirs(ctx, "arm")
		p.Target.Android_arm64.Srcs = makeAndroidSrcs("aarch64")
		p.Target.Android_arm64.Export_include_dirs = androidLibcxxHeaderDirs(ctx, "aarch64")
		p.Target.Android_riscv64.Srcs = makeAndroidSrcs("riscv64")
		p.Target.Android_riscv64.Export_include_dirs = androidLibcxxHeaderDirs(ctx, "riscv64")
		p.Target.Android_x86.Srcs = makeAndroidSrcs("i386")
		p.Target.Android_x86.Export_include_dirs = androidLibcxxHeaderDirs(ctx, "i386")
		p.Target.Android_x86_64.Srcs = makeAndroidSrcs("x86_64")
		p.Target.Android_x86_64.Export_include_dirs = androidLibcxxHeaderDirs(ctx, "x86_64")
		p.Target.Linux_bionic_arm64.Srcs = makeAndroidSrcs("aarch64")
		p.Target.Linux_bionic_arm64.Export_include_dirs = androidLibcxxHeaderDirs(ctx, "aarch64")
		p.Target.Linux_bionic_x86_64.Srcs = makeAndroidSrcs("x86_64")
		p.Target.Linux_bionic_x86_64.Export_include_dirs = androidLibcxxHeaderDirs(ctx, "x86_64")
		p.Target.Glibc_x86.Srcs = makeHostSrcs("i386-unknown-linux-gnu")
		p.Target.Glibc_x86.Export_include_dirs = hostLibcxxHeaderDirs(ctx, "i386-unknown-linux-gnu")
		p.Target.Glibc_x86_64.Srcs = makeHostSrcs("x86_64-unknown-linux-gnu")
		p.Target.Glibc_x86_64.Export_include_dirs = hostLibcxxHeaderDirs(ctx, "x86_64-unknown-linux-gnu")
		p.Target.Linux_musl_x86.Srcs = makeHostSrcs("i686-unknown-linux-musl")
		p.Target.Linux_musl_x86.Export_include_dirs = hostLibcxxHeaderDirs(ctx, "i686-unknown-linux-musl")
		p.Target.Linux_musl_x86_64.Srcs = makeHostSrcs("x86_64-unknown-linux-musl")
		p.Target.Linux_musl_x86_64.Export_include_dirs = hostLibcxxHeaderDirs(ctx, "x86_64-unknown-linux-musl")
		p.Target.Linux_musl_arm64.Srcs = makeHostSrcs("aarch64-unknown-linux-musl")
		p.Target.Linux_musl_arm64.Export_include_dirs = hostLibcxxHeaderDirs(ctx, "aarch64-unknown-linux-musl")
		...
		if (hasDarwinClangPrebuilt(ctx)) {
			p.Target.Darwin.Srcs = []string{":libc++abi_darwin"}
		}
...
func llvmPrebuiltLibraryStatic(ctx android.LoadHookContext) {
	...
	if moduleName == "libc++_static" {
		...
		makeHostSrcs := func(triple string) []string {
			return []string{path.Join(clangDir, "lib", triple, "libc++.a")}
		}
		p.Target.Android_arm.Srcs = makeAndroidSrcs("arm")
		p.Target.Android_arm.Export_include_dirs = androidLibcxxHeaderDirs(ctx, "arm")
		p.Target.Android_arm64.Srcs = makeAndroidSrcs("aarch64")
		...
		p.Target.Linux_musl_x86_64.Srcs = makeHostSrcs("x86_64-unknown-linux-musl")
		p.Target.Linux_musl_x86_64.Export_include_dirs = hostLibcxxHeaderDirs(ctx, "x86_64-unknown-linux-musl")
		p.Target.Windows_x86.Srcs = makeHostSrcs("i686-w64-windows-gnu")
		p.Target.Windows_x86.Export_include_dirs = hostLibcxxHeaderDirs(ctx, "i686-w64-windows-gnu")
		p.Target.Windows_x86_64.Srcs = makeHostSrcs("x86_64-w64-windows-gnu")
		p.Target.Windows_x86_64.Export_include_dirs = hostLibcxxHeaderDirs(ctx, "x86_64-w64-windows-gnu")
		if (hasDarwinClangPrebuilt(ctx)) {
