...
type archProps struct {
	...
	Linux_bionic_x86    archInnerProps
...
func llvmPrebuiltLibraryStatic(ctx android.LoadHookContext) {
	...
	p.Target.Linux_bionic_x86.Srcs = []string{path.Join(libDir, "i386", name)}
...
func libClangRtPrebuiltLibraryShared(ctx android.LoadHookContext, libProps *prebuiltLibraryProps,
	...
	p.Target.Linux_bionic_x86.Srcs = []string{path.Join(libDir, name+"-i686-android"+suffix+".so")}
	p.Target.Linux_bionic_x86.Stem = proptools.StringPtr(name + "-i686-android" + suffix)
...
func libClangRtPrebuiltLibraryStatic(ctx android.LoadHookContext, libProps *prebuiltLibraryProps) {
	...
	p.Target.Linux_bionic_x86.Srcs = []string{path.Join(libDir, name+"-i686-android"+suffix+".a")}
