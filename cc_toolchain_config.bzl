...
load("//__generated_by_bp2build__:cc_toolchain_vars.bzl", "constants")
...
def _compiler_flag_features(flags = [], os_is_device = False):
    ...
    # These are always the last flag group.
...
def is_target_os_device(ctx):
    # TODO: Add more OSes as we support building them.
    android_os_constraint = ctx.attr._android_os_constraint[platform_common.ConstraintValueInfo]
    linux_os_constraint = ctx.attr._linux_os_constraint[platform_common.ConstraintValueInfo]
    if ctx.target_platform_has_constraint(android_os_constraint):
        return True
    elif ctx.target_platform_has_constraint(linux_os_constraint):
        return False
    else:
        fail("Cannot determine if target OS is host or device.")
