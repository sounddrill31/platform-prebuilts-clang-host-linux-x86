...
load(":cc_toolchain_constants.bzl", _actions = "actions", _flags = "flags", _generated_flags = "generated_flags")
...
def _compiler_flag_features(flags = [], os_is_device = False):
    ...
    compiler_flags.extend(_generated_flags.CommonGlobalCflags)

    if os_is_device:
        compiler_flags.extend(_generated_flags.DeviceGlobalCflags)
    else:
        compiler_flags.extend(_generated_flags.HostGlobalCflags)
    ...
    cpp_only_flags = []
    cpp_only_flags.extend(_generated_flags.CommonGlobalCppflags)
    if os_is_device:
        cpp_only_flags.extend(_generated_flags.DeviceGlobalCppflags)
    else:
        cpp_only_flags.extend(_generated_flags.HostGlobalCppflags)

    # Default C compile action only flags (No C++)
    c_only_flags = []
    c_only_flags.extend(_flags.c_compiler_flags)
    c_only_flags.extend(_generated_flags.CommonGlobalConlyflags)
    ...
    features.append(feature(
        ...
        flag_sets = [
            flag_set(
                ...
                flag_groups = [
                    flag_group(
                        flags = _generated_flags.NoOverrideGlobalCflags,
                    ),
                ],
            ),
        ],
...
def _cc_toolchain_config_impl(ctx):
    ...
    builtin_include_dirs.extend(_generated_flags.CommonGlobalIncludes)
    ...
    if os_is_device:
        linker_flags.extend(_generated_flags.DeviceGlobalLldflags)
        linker_flags.extend(_flags.bionic_linker_flags)
    else:
        linker_flags.extend(_generated_flags.HostGlobalLldflags)
