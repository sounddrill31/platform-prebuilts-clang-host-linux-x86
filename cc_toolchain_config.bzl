...
load("//build/soong/bazel/rules:static_libc.bzl", "LibcConfigInfo")
...
def _tool_paths(clang_version_info):
    return [
        ...
        tool_path(
            name = "ld",
            path = clang_version_info.directory.basename + "/bin/llvm-link",
        ),
        tool_path(
            name = "ar",
...
def android_cc_toolchain(
    ...
    # Create the filegroups needed.
    native.filegroup(
        name = "%s_compiler_clang_includes" % name,
        srcs =
            # TODO(jcater): This should come from the clang_version provider.
            # Instead, it's hard-coded because this is a macro, not a rule.
            native.glob(["clang-r370808b/lib64/clang/*/include/**"]),
    )
    if libc:
        native.filegroup(
