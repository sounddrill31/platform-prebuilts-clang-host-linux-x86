...
_DISABLE_RELOC = "disable_pack_relocations"
...
def _relocations_features_test_impl(ctx):
    ...
    for action in actions:
        if action.mnemonic == "CppLink":
            ...
            disable_reloc_arg_set = sets.make(_DISABLE_RELOC_ARGS)
            ...
            elif not sets.disjoint(android_relr_arg_set, action_arg_set):
                # can use disjoint here because ANDROID_RELR and SHT_RELR share an argument
                if sets.contains(action_arg_set, "-Wl,--use-android-relr-tags"):
                    analysistest.fail(env, "action unexpectedly contained ANDROID_RELR arg; arguments: %s" % (action.argv))
                else:
                    analysistest.fail(env, "action unexpectedly contained SHT_RELR arg; arguments: %s" % (action.argv))
            ...
            if _DISABLE_RELOC in ctx.attr.expected_relocation_features:
                asserts.true(
                    env,
                    sets.is_subset(disable_reloc_arg_set, action_arg_set),
                    "action was expected to contain DISABLE_RELOC arg, but did not; arguments: %s" % (action.argv),
                )
            else:
                asserts.true(
                    env,
                    sets.disjoint(disable_reloc_arg_set, action_arg_set),
                    "action unexpectedly contained DISABLE_RELOC arg; arguments: %s" % (action.argv),
                )

    asserts.true(
        env,
        found_link_action,
        "did not find link action; actions: %s" % (actions),
    )
...
def _test_relocation_features(*, name, sdk_version, expected_features, extra_target_features = []):
    ...
    if _DISABLE_RELOC in expected_features:
        expected_linux_features = expected_features
    else:
        expected_linux_features = []

    relocation_linux_features_test(
        name = linux_test_name,
        target_under_test = target_under_test,
        expected_relocation_features = expected_linux_features,
    )
    return [
        android_test_name,
        linux_test_name,
...
def _generate_relocation_feature_tests():
    return (
        ...
        _test_relocation_features(
            ...
            sdk_version = "30",
            expected_features = [_DISABLE_RELOC],
