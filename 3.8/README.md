...
merge back a change.
  ...
  **Note**: If there were changes to *aosp/master* before the squashed commit,
  grab those changes (using step 2), before applying this step, and finally
  repeat step 2 for changes after the squashed commit.
...
9. Test everything before committing the patch from the previous step.
...
1. Build all of llvm/clang/compiler-rt.
        ...
        cd $ANDROID_BUILD_TOP/external/llvm && mma -j32 \
        && cd ../clang && mma -j32 \
        && cd ../compiler-rt && mma -j32
...
This includes clang itself!
        ...
        # Note that we have to do the explicit build in llvm to pick up
        # all of the necessary llvm-* utilities too.
...
1. Do a Clang-only build and ensure it is not broken:
        ...
        USE_CLANG_PLATFORM_BUILD=true m -j32
